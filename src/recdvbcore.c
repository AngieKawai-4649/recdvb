...
int open_frontend(int dev_num)
{
	...
	snprintf(device, DEVNAME_BUFFER, "/dev/dvb/adapter%d/frontend0", dev_num);
...
int frontend_tune(int fefd, unsigned int freq, unsigned int tsid, int lnb)
{
	int isdbtype;
	...
	/* frequency */
	prop[props.num].cmd = DTV_FREQUENCY;
	prop[props.num].u.data = freq;
...
int open_demux(int dev_num)
{
	...
	snprintf(device, DEVNAME_BUFFER, "/dev/dvb/adapter%d/demux0", dev_num);
...
int open_dvr(int dev_num)
{
	...
	snprintf(device, DEVNAME_BUFFER, "/dev/dvb/adapter%d/dvr0", dev_num);
