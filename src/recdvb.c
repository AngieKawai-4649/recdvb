...
static int parse_options(struct recdvb_options *opts, int argc, char **argv)
{
	...
	if(rtn == CH_RETURN_NOTFOUND){
		fprintf(stderr, "Error: Channel %s is not found.\n", channel);
		return(-1);
	}
...
int main(int argc, char **argv)
{
	...
	memset(&opts, 0, sizeof(struct recdvb_options));
	...
	while (!f_exit) {
		...
		for (i = 0; i < nfds; ++i) {
			...
			if (evs[i].data.fd == sfd) {
				...
				fprintf(stderr, "\nInfo: Catch signal. %zd\n", r);
				f_exit = 1;
				break;
			} else if (evs[i].data.fd == tfd) {
				/* timer */
				uint64_t exp;
				ssize_t r = read(tfd, &exp, sizeof(uint64_t));
				...
				if (tuned == 0) {
					...
					f_exit = 1;
					fprintf(stderr, "Error: Tune timeout. %zd\n", r);
					break;
				} else {
#ifdef LOGOUTPUT
					uint64_t w_byte = 0;
